...
Expected<void> ParameterStorage::setStrVector(gxf_uid_t uid, const char* key, const char** value,
  ...
  for (uint64_t i = 0; i < length; i++) {
    if (!value[i]) {
      GXF_LOG_ERROR("Value at index %" PRId64 " is null for vector param: %s", i, key);
      return Unexpected{GXF_ARGUMENT_NULL};
    }
  ...
  for (const auto& kvp : parameters_) {
    for (const auto& kvp2 : kvp.second) {
      ...
      if (kvp2.second->isMandatory() && !kvp2.second->isAvailable()) {
        ...
        GxfEntityGetName(context_, eid, &entity_name);
        GXF_LOG_ERROR(
            "Mandatory parameter [%s] not set in component [%s] with uid "
            "[%" PRId64 "] in entity [%s]",
            kvp2.first.c_str(), component_name, kvp.first, entity_name);
    ...
    if (kvp.second->isMandatory() && !kvp.second->isAvailable()) {
      ...
      GxfEntityGetName(context_, eid, &entity_name);
        GXF_LOG_ERROR(
            "Mandatory parameter [%s] not set in component [%s] with uid "
            "[%" PRId64 "] in entity [%s]",
            kvp.second->key(), component_name, uid, entity_name);
      return Unexpected{GXF_PARAMETER_MANDATORY_NOT_SET};
    }
  }
