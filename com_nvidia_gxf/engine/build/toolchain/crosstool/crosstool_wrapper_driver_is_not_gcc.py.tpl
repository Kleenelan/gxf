...
def InvokeNvcc(argv, log=False):
  ...
  if len(out_file) != 1:
    return 1
  # Debug mode uses -Og. Check for numeric values as well as optimization level
  # See the NVCC documentation for acceptable Optimization levels
  opt = (' -O' + opt_option[0] if (len(opt_option) > 0 and opt_option[0].isdigit()
                                   and int(opt_option[0]) > 0)
         else ' -g -G')
  ...
  supported_cuda_compute_capabilities = [ %{cuda_capabilities} ]
  ...
  cmd = 'PATH=' + os.path.dirname(CPU_COMPILER) + ':$PATH ' + cmd
  if log: Log(cmd)
